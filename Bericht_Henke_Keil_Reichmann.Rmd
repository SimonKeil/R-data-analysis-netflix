...
# wie bin ich mir duplikaten vorgegangen?
...
#die Spalten, die mich interessieren
collumns <- raw_data %>%
...
# eindeutiger film + genre
col_genre <- collumns %>%
  select(Genre, year, Title)%>% 
  separate_rows(Genre, sep = ", ")%>%
  distinct(.keep_all = TRUE)

#eindeutiger film + land
col_country <- collumns %>%
  select(year, Title, country)%>% 
  separate_rows(country, sep = ",")%>%
  distinct(.keep_all = TRUE)

# beide werden gejoint, dabei werden immer die zeilen der einduetigen filme verbunden
final <- full_join(col_genre, col_country, by = c("year", "Title"))

# hats geklappt? 
col_genre%>%filter(Title == "Life")
col_country%>%filter(Title == "Life")
final%>%filter(Title == "Life")
final
final%>%drop_na()
# jup :)
```
...
*Anmerkung zu Beginn der Analyse:* Wir lesen vor der eigentlichen Auswertung die Daten ein und reduzieren sie auf den Grund-Datenbestand, den wir im Folgenden nutzen wollen: Die Spalten mit dem Titel, Genres, Veröffentlichungsdatum und Länder Verfügbarkeit. Dabei entfernen wir zudem alle Zeilen, in denen Angaben fehlen, sowie die Filme/Serien mit Veröffentlichungsdatum vor 1965 und nach 2020. In diesen Jahren sind nämlich nur sehr wenige Einträge vorhanden, sodass keine sinnvollen Aussagen getroffen werden können.
...
Diese Beobachtung sieht man auch, wenn man die Zahl der Filme in dem jeweiligen Veröffentlichungsjahr darstellt. Da der Zusammenhang annähernd exponentiell ist, ist die Anzahl im Plot mit einer logarithmischen Skala versehen.
...
```
Länder in unserem Datensatz. Eine interessante Kenngröße ist, wie viele Filme und Serien es jeweils in den Ländern gibt.
...
data_country %>%
  ...
  geom_point()+
  labs(title = "Anzahl verfügbarer Filme & Serien in den Ländern",
       x = "Anzahl der Inhalte", 
       y = "Land")
```

Man sieht, dass es starke Unterschiede zwischen den einzelnen Ländern gibt und dass keine Gruppierungen festzustellen sind.

Eine nahe liegende Folgerung wäre, dass die Auswahl an Filmen und Serien in vielen Ländern ähnlich ist. Das würde bedeuten, dass die meisten Inhalte in fast allen Ländern verfügbar sind.
...
data_country %>% 
  ...
  labs(title = "In wievielen Ländern sind Inhalte verfügbar?",
       x = "In so vielen Ländern verfügbar",
       y = "Anzahl Filme & Serien")
```
Tatsächlich ist ein Großteil der Medien nur in einem einzigen Land abrufbar, das Angebot von Netflix scheint also recht speziell auf einzelne Länder zugeschnitten zu sein. Nur im Bereich von knapp 35 Ländern ist ein kleinerer Spike mit fast 1500 sehr international verfügbaren Inhalten.
...
count_genre_vs_year %>%
  ggplot(aes(x = year, y = n)) +
  geom_point() +
  labs(title = "Anzahl der Genres, in denen Inhalte verfügbar sind nach Veröffentlichungsjahr",
       x = "Veröffentlichungsjahr",
       y = "Anzahl der Genres")
```

Allerdings wird in unserem Datensatz ein Film/einer Serie i.d.R. mehreren Genres zugeordnet. Liegt der Zusammenhang also eventuell daran, dass neuere Inhalte einfach mehr unterschiedlichen Genres zugeordnet werden? Um das zu beantworten plotten wir die durchschnittliche Zahl der Genres, denen ein Medium in einem gegebenen Jahr zugeordnet wird:
